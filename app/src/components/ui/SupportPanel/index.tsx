...
const SupportPanelItem: FC<SupportPanelItemProps> = ({ item, onClick }) => {
  ...
  if (Array.isArray(itemCollection.items)) {
    ...
    return (
      <SupportPanelButton className={`${styles.button} ${styles.button_coll}`} type="button" onClick={clickHandler}>
  ...
  return (
    <SupportPanelButton className={styles.button} type="link" href={singleItem.url}>
      {singleItem.title}
    </SupportPanelButton>
  );
};
...
const SupportPanel: FC<Props> = ({ data }) => {
  ...
  const supportCollectionClickHandler = (coll: ISupportCollection) => {
    const sortedColl = {
      ...coll,
      items: [...coll.items],
    };

    sortedColl.items.sort((a, b) => a.order - b.order);
      ...
      <div className={styles.layout}>
        <div className={styles.container}>
          <div className={`${styles.heading} ${styles.heading_shift}`}>
            <GhostButton className={styles['return-button']} type="button" onClick={() => setColl(null)}>
              <img className={styles['return-button_icon']} src={ArrowBack} alt="" />
            </GhostButton>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.items}>
          {sortedData.map((item, id) => (
            <SupportPanelItem key={id} item={item} onClick={supportCollectionClickHandler} />
          ))}
        </div>
      </div>
    </div>
