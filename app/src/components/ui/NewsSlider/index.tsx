      ...
      <Slider>
        {data.map((item, id) => (
          <Slide key={id} index={id}
            ...
            innerClassName={style.slide_inner}>
              <NewsCard data={item} />
          </Slide>
        ))}
      </Slider>
      <div className={slideControlsStyle.container}>
        <ButtonBack className={slideControlsStyle.arrow_prev}>
          {""}
        </ButtonBack>
        {settings.showDots && (
          <DotGroup
            className={slideControlsStyle.dots}
            showAsSelectedForCurrentSlideOnly
            />
        )}
        <ButtonNext className={slideControlsStyle.arrow_next}>
          {""}
        </ButtonNext>
      </div>
    </CarouselProvider>
